...
// 10.反转链表
func (this *List) ReverseList(head *Node) *Node{
	cur := head
	var pre *Node = nil
	for cur != nil {
...
func (this *List) mergeTwoLists(l1 *listNode, l2 *listNode) *listNode{
	// 如果有一条链是nil,直接返回另一条链
	if l1 == nil{
		return l2
	}
	...
	// 定义一个结果节点
	var res *listNode
	// 当l1节点的值大于l2节点的值, 那么res指向l2的节点, 从l2开始遍历,反之从l2开始遍历
	if l1.value >= l2.value {
		res = l2
		res.next = this.mergeTwoLists(l1, l2.next) // 使用递归, 不断去找两个链表中比较小的元素, 然后result接上那个元素
	} else {
		res = l1
